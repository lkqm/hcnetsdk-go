...
type ExceptionCallBack func(dwType uint32, lUserID int32, lHandle int32, pUser unsafe.Pointer)
...
func fExceptionCallBackGo(dwType C.DWORD, lUserID C.LONG, lHandle C.LONG, pUser *C.void) {
	...
	if callback != nil {
		callback(uint32(dwType), int32(lUserID), int32(lHandle), unsafe.Pointer(pUser))
	}
}
...
type RealDataCallBack func(lPlayHandle int32, dwDataType uint32, pbuffer []byte, dwBufSize uint32, pUser unsafe.Pointer)
...
func fRealDataCallBackGo(lPlayHandle C.LONG, dwDataType C.DWORD, pBuffer *C.BYTE, dwBufSize C.DWORD, pUser *C.void) {
	...
	callback := realDataCallBackHooks[userId]
	if callback != nil {
		bytes := C.GoBytes(unsafe.Pointer(pBuffer), C.int(dwBufSize))
		callback(int32(lPlayHandle), uint32(dwDataType), bytes, uint32(dwBufSize), unsafe.Pointer(pUser))
	}
}
